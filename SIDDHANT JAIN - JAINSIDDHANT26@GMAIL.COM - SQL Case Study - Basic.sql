...
SELECT COUNT(*) 'TOTAL NO OF ROWS IN TRANSACTIONS' FROM Transactions

SELECT COUNT(*) 'TOTAL NO OF ROWS IN CUSTOMER' FROM Customer

SELECT COUNT(*) 'TOTAL NO OF ROWS IN PROD_CAT_INFO' FROM prod_cat_info
...
/* I changed the date type when inserting data in the database and used Edit Mapping to do so. */
...
SELECT top 3 city_code, COUNT(customer_id) AS 'Number of Customer ' FROM Customer
	GROUP BY city_code 
...
FROM ( SELECT T.*, MAX(T.tran_date) OVER () AS MAX_TRANS_DATE FROM Transactions T
	  )
T 
JOIN prod_cat_info P 
	ON T.prod_cat_code = P.prod_cat_code AND T.prod_subcat_code = P.prod_sub_cat_code
WHERE T.tran_date < T.MAX_TRANS_DATE AND
	  T.tran_date > DATEADD(M,-3,T.MAX_TRANS_DATE) AND
	  T.total_amt < 0 
GROUP BY P.prod_cat
